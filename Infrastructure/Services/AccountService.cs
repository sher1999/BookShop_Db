...
public class AccountService : IAccountService
{
    ...
    {
        var existing = _context.Users.Where(x => x.PhoneNumber == registerDto.PhoneNumber).FirstOrDefault();
        if (existing ==null)
        {
        ...
        return new Response<IdentityResult>(HttpStatusCode.BadRequest,
            new List<string>() { "PhoneNumber   already exists" });
    ...
    public async Task<Response<List<UserDto>>> ViewProfile(string email,string password)
    {
        
        var users =await _context.Users.Where(x=>x.Email==email).Select(x=>new UserDto()
            {
                Id=x.Id,
                Username = x.UserName,
                PhoneNumber = x.PhoneNumber,
    ...
    public async Task<Response<string>> DeleteRole(DeleteRoleDto model)
    {
        var role = await _context.Roles.FirstOrDefaultAsync(x=>x.Name.ToUpper() ==model.RoleName.ToUpper());
        var user = await _context.Users.FindAsync(model.UserId);
        await _userManager.RemoveFromRoleAsync(user, role.Name);
        return new Response<string>("Remove shud");
